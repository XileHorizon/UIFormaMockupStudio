...
function ModifierEditor({ modifier }: { modifier: LayoutModifier }) {
  ...
  if (modifier.type === 'radial') {
    ...
    return <div style={{ marginTop: 7, padding: 8, border: '1px solid var(--border)', borderRadius: 7, background: 'var(--surface)' }}>
      ...
      <Slider label="Angle" value={settings.angle} min={0} max={360} onChange={angle => setSettings({ ...settings, angle })} />
      <Slider label="Start" value={settings.startAngle} min={-180} max={180} onChange={startAngle => setSettings({ ...settings, startAngle })} />
