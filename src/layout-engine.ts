...
function toTransform(p: THREE.Vector3, q: THREE.Quaternion, scale: number): Transform {
  const e = new THREE.Euler().setFromQuaternion(q, 'XYZ')
  return { posX: p.x, posY: p.y, posZ: p.z, rotX: THREE.MathUtils.radToDeg(e.x), rotY: THREE.MathUtils.radToDeg(e.y), rotZ: THREE.MathUtils.radToDeg(e.z), scale }
}
...
function radialTransforms(input: Transform, settings: RadialLayoutSettings): Transform[] {
  ...
  const divisor = Math.abs(settings.angle) >= 359.999 ? count : Math.max(1, count - 1)
  return Array.from({ length: count }, (_, index) => {
    ...
    return toTransform(p, r, input.scale)
  })
}
